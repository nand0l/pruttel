...
import time
import os
st.title('Invoke Anthropic Claude 3 Sonnet with Streamlit')

prompt_start = ".\\pages\\sources\\prompt-start.txt"
...
modelid = st.radio(
    "Select a ModelID",
    ["Sonnet", "Haiku"],
    captions=["Claude is the most powerful model and excels at complex reasoning tasks such as sophisticated dialogue or detailed content creation.", "Claude 3 Haiku is one of the most affordable and fastest options on the market for its intelligence category.", "Never stop learning."])

if modelid == 'Sonnet':
    st.write('You selected Sonnet.')
    model_id = "anthropic.claude-3-sonnet-20240229-v1:0"
else:
    st.write("You selected Haiku")
...
# Streamlit UI
if len(st.session_state["input_file_name"]) > 0:
